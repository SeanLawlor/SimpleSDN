...
import net.floodlightcontroller.fyp.InfoCollector;
...
class OFChannelHandler extends SimpleChannelInboundHandler<Iterable<OFMessage>> {
	...
	public abstract class OFChannelState {
		...
		void processOFMessage(OFMessage m)
				throws IOException {
			/*
			 * Process message for injected packets before other modules.
             *  -> Echo replies are not passed on to modules 
             *  -> Modules can stop other modules from processing 
             *  -> Processing as soon as possible allows more accurate time reading
			 */
			try{
				InfoCollector sc = InfoCollector.getInstance();
				
				//These silently fail if message not relevant
				sc.processEchoResponse(connection.getDatapathId(), m);
				sc.processPacket(connection.getDatapathId(), m);
			}catch(Exception e){
				//Not yet ready for handling
			}
